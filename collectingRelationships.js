...
for(i=0; i<allUsers.length; i++){
	...
	var sameCity = allUsers[i];
	if(joeyProfile.city === sameCity.city){
		joeyProfile.friends.push(sameCity.username);
		console.log(joeyProfile.friends);
	}
}

for(i=0; i<allUsers.length; i++){
	var sameJob = allUsers[i];
	if(joeyProfile.job === sameJob.job){
 	joeyProfile.coworkers.push(sameJob.username);
...
for(i=0; i<allUsers.length; i++){
	...
	var joeyName = joeyProfile.username;
	var splitJoeyName = joeyName.split(" ");
	var plain = splitJoeyName[1];

	var allUsersUnite = allUsers[i].username;
	var splitName = allUsersUnite.split(" ");
	var plain2 = splitName[1];
	if(plain2 === plain){
		joeyProfile.family.push(allUsersUnite);
	}
